...
function printMap(map: TileMap) {
  const text = map.map((r) => r.map((t) => fmtSymbol(t)).join("")).join("\n");
...
function printMapDist(map: TileMap, pad: number) {
  const text = map
    .map((r) => r.map((t) => fmt(t.distance).padStart(3)).join(""))
    .join("\n");
...
function printMapPos(map: TileMap) {
  const text = map.map((r) => r.map((t) => fmtPosition(t)).join("")).join("\n");
...
function fmtPosition(tile: Tile) {
  switch (tile.position) {
    case TilePosition.INSIDE:
      return "I";
    case TilePosition.OUTSIDE:
      return "O";
    case TilePosition.PIPE:
      return "-";
    case undefined:
      return ".";
    default:
