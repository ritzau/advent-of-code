...
function walk(map: Map, minStraight: number, maxStraight: number) {
    ...
    const ns = map[map.length - 1][map[0].length - 1].nodes.flat()
    ...
    function getNode(x: number, y: number, direction: Direction, straight: number) {
        const tile = map[y][x]
...
function createNode(x: number, y: number, direction: Direction, straight: number, distance: number) {
    return {
        distance,
        visited: false,
        from: undefined,
...
function printMap(map: Map) {
    console.log(map.map((l, y) => l.map((t, x) => {
