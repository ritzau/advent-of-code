...
function part1(path: string) {
  ...
  const map = Array.from({ length: bb.maxY - bb.minY + 1 }, () =>
    Array.from({ length: bb.maxX - bb.minX + 1 }, () => "."),
  );
  ...
  return map.reduce(
    (a, line) =>
      a + line.reduce((count, c) => (c !== outer ? count + 1 : count), 0),
    0,
  );
}
    ...
    switch (poi.kind) {
      ...
      case "end":
        const index = marks.findIndex(
          (elm) => elm.kind === "vertical" && elm.column === poi.column,
        );
        if (index !== -1) {
...
function countHoles(
  previousLine: LineMark[],
  currentLine: LineMark[],
  nextLine: LineMark[] | undefined,
) {
  ...
  if (previousMark.kind === "horizontal") {
    ...
    if (
      !(
        hasOverlap(previousMark, previousLine) &&
        hasOverlap(previousMark, nextLine)
      )
    ) {
      inside = !inside;
    }
...
function hasOverlap(
  { start, end }: HorizontalLineMark,
  pois: LineMark[] | undefined,
) {
  if (pois === undefined) {
    return false;
  }
  ...
  const result = pois.some((poi) => {
    switch (poi.kind) {
      ...
      case "horizontal":
        return (
          (start <= poi.start && end > poi.start) ||
          (start <= poi.end && end > poi.end)
        );
      default:
        assert.fail();
    }
