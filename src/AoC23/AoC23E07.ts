...
function parseHand(handString: string) {
    ...
    const frequency = Array.from(handString).reduce((m, c) => m.set(c, (m.get(c) ?? 0) + 1), new Map<string, number>())
    const frequencyString = Array.from(frequency.values()).sort().reverse().join('')
