...
async function part1a(path: string) {
    ...
    for (const pattern of patterns) {
        ...
        let c = 0
        for (const combination of combinations(pattern.template, pattern.runLengths)) {
...
async function part2(path: string) {
    ...
    for (const { template, runLengths } of patterns) {
        ...
        const t2 = `${t}?${t}?${t}?${t}?${t}`
        const rl2 = runLengths.concat(runLengths, runLengths, runLengths, runLengths)
        console.log(t)
        const c = combo3({
...
function* combinations(template: string[], runLengths: number[]) {
    ...
    while (true) {
        ...
        } else if (t.filter(ch => ch === '?' || ch === '#').length < totalHashes) {
            // Ignore
        } else {
...
enum MatchResult { YES, MAYBE, IMPOSSIBLE }
...
function isMatching(pattern: string[], runLengths: number[]) {
    ...
    const result = runLengthIndex === runLengths.length - 1 ? MatchResult.YES : MatchResult.IMPOSSIBLE
    // console.log(pattern.join(''), result)
    return result
}

type Combo3State = { template: string[], runLengths: number[], runLengthCount: number }
...
function combo3(state: Combo3State, prefix = ''): number {
        ...
        if (runLengthCount > 0) {
            ...
            if (ch === '?') {
                if (runLengthIndex >= runLengths.length) {
                    ...
                    }, prefix + template.slice(0, position).join('') + '#') + combo3({
                        template: template.slice(position + 1),
                        runLengthCount: 0,
                        runLengths: runLengths.slice(runLengthIndex)
    ...
    const hasSeenAllGroups = runLengthIndex === runLengths.length && runLengthCount === 0
    const isInLastGroupAndSeenAllHashes = runLengthIndex === runLengths.length - 1 && runLengthCount === runLengths[runLengthIndex]
