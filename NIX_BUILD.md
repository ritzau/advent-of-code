...
Uses `stdenv.mkDerivation` with Nim compiler:
...
**Requirements:**
- Simple Nim files with local imports
- No external Nimble packages for AoC solutions

**Build output:**
- `result/bin/part1`
- `result/bin/part2`

**Note:** For projects with Nimble dependencies, nixpkgs provides `buildNimPackage` which uses lockfiles generated by the `nim_lk` utility. For AoC's self-contained programs, direct compilation with `mkDerivation` is simpler.
